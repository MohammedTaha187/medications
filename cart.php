...
if (isset($_GET['delete'])) {
    ...
    $delete_cart_item->execute([$delete_id, $user_id]); // إضافة user_id لزيادة الأمان
...
if (isset($_POST['update_qty'])) {
    ...
    $p_qty = $_POST['p_qty'];
    $p_qty = filter_var($p_qty, FILTER_SANITIZE_NUMBER_INT); // تأكيد أن الكمية رقمية
    $update_qty = $conn->prepare("UPDATE `cart` SET quantity = ? WHERE id = ? AND user_id = ?");
    $update_qty->execute([$p_qty, $cart_id, $user_id]); // إضافة user_id لزيادة الأمان
